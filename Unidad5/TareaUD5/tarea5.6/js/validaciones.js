...
function checkNum(objeto,min) {
    ...
    if (!objeto.value.trim()) {
        ...
        return false;        
    } else if (objeto.value<min) {
        objeto.style.border = "2px solid red";
        error.style.color = "red";
        error.innerHTML = `${objeto.name} no puede ser inferior a ${min}`;
        return false;         
    } else {
        objeto.style = "none"; //Para resetear estilo!!!!
        error.innerHTML = "";
...
function checkDate(objeto) {
  ...
  }else if (!/^[0-3][0-9]\/[0-1][0-9]\/[0-9]{4}$/.test(objeto.value)) {
